...
type taskDispatchDAGServiceSuite struct {
	suite.Suite

	items []TaskQueueItem
}

func TestTaskDispatchDAGServiceSuite(t *testing.T) {
	suite.Run(t, new(taskDispatchDAGServiceSuite))
}

func (s *taskDispatchDAGServiceSuite) SetupTest() {
...
func (s *taskDispatchDAGServiceSuite) TestSingleHostTaskGroupsBlock() {
	...
	service := newDistroTaskDAGDispatchService("distro_1", items, time.Minute)
	spec := TaskSpec{
		Group:        "group_1",
		BuildVariant: "variant_1",
		Version:      "version_1",
...
func (s *taskDispatchDAGServiceSuite) TestFindNextTask() {
	service := newDistroTaskDAGDispatchService("distro_1", s.items, time.Minute)
	var spec TaskSpec
